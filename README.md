# <div align="center">🚀 Jujaru Govardhan</div>

<div align="center">

![banner](https://capsule-render.vercel.app/api?type=waving\&color=0:0f2027,50:203a43,100:2c5364\&height=200\&section=header\&text=Jujaru%20Govardhan\&fontSize=40\&fontColor=ffffff)

💻 Frontend Developer | React.js Specialist | 3+ Years Experience
📍 Hyderabad, India

</div>

---

## 🌑 About Me

Frontend Developer with **3+ years of experience** building scalable and responsive web applications using **React.js, Redux Toolkit, and REST APIs**.

* ⚡ Strong in JavaScript (ES6+)
* 🎯 Focused on clean & maintainable code
* 🚀 Experience in performance optimization
* 🤝 Agile team collaboration

---

## 🛠️ Tech Stack

🎨 Frontend  <br>
HTML5          ██████████████████░░ 90%
CSS3           █████████████████░░░ 85%
JavaScript     ██████████████████░░ 90%
React.js       █████████████████░░░ 85%
Redux Toolkit  ████████████████░░░░ 80%
Next.js        ██████████████░░░░░░ 75%

⚙️ Backend
Node.js        ██████████████░░░░░░ 75%
Express js     ██████████████░░░░░░ 75%
Express.js     █████████████░░░░░░░ 70%

🚀 Frameworks & Tools
Git & GitHub   █████████████████░░░ 85%


## 💼 Experience

**UI / React JS Developer**
📍 Resourceone IT Solutions, Hyderabad
📅 June 2022 – Present

* Built responsive and scalable web apps
* Developed reusable React components
* Used React Hooks for state management
* Integrated REST APIs
* Improved UI/UX and performance

---

## 🎓 Education

🎓 **BSc - Electronics (86%)**
Sri Krishnadevaraya University, Anantapur (2021)

---

## 🚀 Projects

### 🏦 US Banking Application

* Secure banking system
* Account management & transactions
* Real-time API integration

---

### 📦 FedEx Shipping & Tracking App

* Shipment tracking system support
* UI issue fixing & performance monitoring
* API integration support

---

### 💬 Real-Time Chat App (MERN)

🔗 https://github.com/Govardhan0020/MernchatApp

* Built using React, Node.js, Express, MongoDB
* Real-time messaging using Socket.IO

---

## 🌐 Connect With Me

* 💼 GitHub: https://github.com/Govardhan0020
* 🌍 Portfolio: https://govardhan0020.github.io/portfolio/
* 📧 Email: [govardhanjpg6940@gmail.com](mailto:govardhanjpg6940@gmail.com)

---

## 🌍 Languages

* Telugu 🇮🇳
* Hindi 🇮🇳
* English 🌍

---

## 🎯 Hobbies

* 📚 Reading tech blogs
* 🎥 Watching coding tutorials
* ✍️ Writing notes
* 🤝 Developer communities

---
